...
<template>
  <div class="flex flex-col print:gap-0 sm:py-8 py-0 print:py-0 gap-6 sm:gap-12" ref="pdfSection">
    <div
      ...
      <h6>
        Būsena: <br />
        <span :class="offer.offer?.status === 'Nepatvirtintas'
            ? 'text-orange-300'
    ...
    <div class="flex justify-evenly flex-wrap gap-8 lg:gap-0 lg:py-4 py-0 print:hidden">
        ...
        <div class="flex gap-2 sm:gap-8 justify-center flex-wrap">
          ...
          <div class="flex flex-col gap-2">
            <a :href="'tel:' + offer.offer?.client?.phone">
              <BaseInput :disable="true" :name="offer.offer?.client?.phone" label="telefono numeris"
                class="pointer-events-none" />
            </a>
            <a :href="'mailto:' + offer.offer?.client?.email">
              <BaseInput :disable="true" :name="offer.offer?.client?.email" label="elektroninis pastas"
                class="pointer-events-none" />
            </a>
          </div>
        </div>
      ...
      <div class="flex flex-col items-center gap-8">
        ...
        <div class="flex gap-2 sm:gap-8 justify-center flex-wrap">
          <div class="flex flex-col gap-2">
            <BaseInput :disable="true" :name="offer.offer?.creator?.username +
          ' ' +
          ...
          <div class="flex flex-col gap-2">
            <a :href="'tel:' + offer.offer?.creator?.phone" class="hover: cursor-pointer">
              <BaseInput :disable="true" :name="offer.offer?.creator?.phone" label="Telefono numeris"
                class="pointer-events-none" />
            </a>

            <a :href="'mailto:' + offer.offer?.creator?.email">
              <BaseInput :disable="true" :name="offer.offer?.creator?.email" label="elektroninis pastas"
                class="pointer-events-none" />
            </a>
          </div>
        </div>
    ...
    <div class="print:flex hidden items-center justify-between">
      ...
      <NuxtImg src="/images/logo.png" alt="Moderni Tvora logotipas" width="129" height="72" decoding="auto"
        loading="lazy" :ismap="true" class="hidden print:block" />
    </div>
    ...
    <div class="flex flex-col pr-4 sm:pr-0">
      ...
      <div class="print:border-b border-dark-full">
        <OfferResult v-for="(result, index) in offer.offer?.results" :key="result._id" :result="result"
          :index="index" />
      </div>
      <p
    ...
    <div class="flex flex-col pr-4 sm:pr-0">
      ...
      <div class="print:border-b border-dark-full">
        <OfferWork v-for="(work, index) in offer.offer?.works" :key="work._id" :work="work" :index="index" />
      </div>
      <p
        class="sm:text-2xl text-lg print:text-base print:font-medium text-center print:bg-transparent p-2 print:pt-0 print:w-fit bg-gray-light rounded-2xl font-semibold w-56 sm:w-96 mt-8 print:mt-1 self-end">
    ...
    <div class="flex justify-end border-dark-full">
      <div class="w-96 print:w-72">
        <div class="text-xl print:text-base flex flex-col print:gap-0 gap-2">
          ...
          <div class="flex px-4 justify-between" :class="!offer.offer?.discount ? 'border-y-2 border-dark-full py-1' : ''
          ">
          ...
          <div v-if="offer.offer?.discount" class="flex px-4 print:py-1 justify-between border-y-2 border-dark-full">
  ...
  <div class="flex flex-col items-center gap-10 mt-8 mb-12 print:hidden">
    ...
    <div v-if="showButtons" class="flex justify-center gap-8 flex-wrap">
      <BaseButton name="pasiūlymas tenkina" @click="confirmHandler" width="w-56 sm:w-96" :isLoading="isLoading" />
      <BaseButton name="pasiūlymas netenkina" @click="cancelHandler" width="w-56 sm:w-96" :isLoading="isLoading" />
    </div>
