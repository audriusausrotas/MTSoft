...
const MONTHS = ["Visi", "Sausis", "Vasaris", "Kovas", "Balandis", "Gegužė", "Birželis", "Liepa", "Rugpjūtis", "Rugsėjis", "Spalis", "Lapkritis", "Gruodis",]
...
const filterBonuses = () => {
    if (filterYear.value === "Visi") {
        if (filterMonth.value === "Visi") {
            ...
            filteredBonuses.value = bonuses.value.filter(item => MONTHS[+item.dateFinished.slice(5, 7)] === filterMonth.value
            )
        }
    } else {
        if (filterMonth.value === "Visi") {
            filteredBonuses.value = bonuses.value.filter(item => +item.dateFinished.slice(0, 4) === +filterYear.value
            )
        } else {
            filteredBonuses.value = bonuses.value.filter(item => MONTHS[+item.dateFinished.slice(5, 7)] === filterMonth.value && +item.dateFinished.slice(0, 4) === +filterYear.value)
        }
    }
    calculateBonuses()
}
...
<template>
    <div class="flex flex-col gap-4">
        <div class="flex gap-8 items-end">
            <BaseSelectField label="Metai" :values="existingYears" id="years" defaultValue="Visi" width="w-60"
                @onChange="(value: string) => yearFilterHandler(value)" />

            <BaseSelectField label="Mėnesiai" :values="MONTHS" id="months" :defaultValue="filterMonth" width="w-60"
        ...
        <div>
            ...
            <div v-for=" bonus, index  in  filteredBonuses " :key="index + bonus.price"
