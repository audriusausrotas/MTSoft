export default function getPriceItem(name: string) {
  ...
  if (backupStore.backupExist) {
    ...
    oldProduct = backupStore.results.find((item) => item.name === name);

    if (!oldProduct)
      oldProduct = backupStore.works.find((item) => item.name === name);
    ...
    if (!oldProduct) {
      ...
      const returnProduct: any = {
        _id: oldProduct.id,
        name: oldProduct.name ? oldProduct.name : oldProduct.name,
