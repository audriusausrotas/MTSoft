...
export default defineEventHandler(async (event) => {
  ...
  if (gamybaExist) {
    ...
    const calculateCenter = (array: BindingItem[]) => {
      if (array.length > 1) {
        ...
        for (const item of array) {
          if (lastElement) {
            ...
            lastElement = item;
            addBindings(item.color, item.lastHeight, "Koja vienguba");
          }
        }
      }
    };

    calculateCenter(front);
    calculateCenter(left);
    calculateCenter(back);
    calculateCenter(right);
