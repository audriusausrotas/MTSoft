  ...
  <div class="flex flex-wrap items-center gap-4 py-4">
    ...
    <div class="flex flex-col gap-2 w-96">
      ...
      <div class="flex gap-2">
        <BaseInput label="spalva" width="w-20" :variant="colorEditable ? 'light' : ''"
          :disable="colorEditable ? false : true" :name="props.result.color"
        ...
        <BaseInput label="savikaina" :name="props.result.cost" width="w-24" variant="light"
    ...
    <div v-if="showAditionalVertical" class="flex flex-col gap-2">
      ...
      <BaseInput v-if="isFenceboard" :name="props.result.space" width="w-24" label="tarpas"
        :variant="spaceEditable ? 'light' : ''" :disable="spaceEditable ? false : true"
        @onChange="(value) => useResults.updateSpace(props.index, value)" />
    </div>

    <div class="flex flex-col gap-2">
      <BaseInput v-if="props.result.direction" width="w-36" label="kryptis" :name="props.result.direction" />
      <BaseInput v-if="showAditionalHorizontal" label="pramatomumas" :disable="true" :name="props.result.seeThrough"
        width="w-36" />

      <BaseInput v-if="isFenceboard && props.result.twoSided" width="w-36" label="dvipusė" :name="props.result.twoSided"
        :disable="false" />
    </div>
    ...
    <NuxtImg src="/icons/delete.svg" width="20" height="20" @click="useResults.deleteResult(props.result.id)"
