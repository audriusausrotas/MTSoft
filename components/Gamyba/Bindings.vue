...
const saveHandler = async (field: string) => {
  ...
  if (response.success) {
    ...
    else if (field === "type") isSavedType.value = true;
    else if (field === "height") isSavedHeight.value = true;
    else if (field === "quantity") isSavedQuantity.value = true;
    else if (field === "color") isSavedColor.value = true;
  } else {
    setError(response.message);
  }
};
...
watch(cut, (newCut) => {
  if (newCut !== props.binding.cut) isSavedCut.value = false;
  else isSavedCut.value = true;
});

watch(done, (newDone) => {
  if (newDone !== props.binding.done) isSavedDone.value = false;
  else isSavedDone.value = true;
});

watch(type, (newType) => {
  if (newType !== props.binding.type) isSavedType.value = false;
  else isSavedType.value = true;
});

watch(height, (newHeight) => {
  if (+newHeight !== +props.binding.height) isSavedHeight.value = false;
  else isSavedHeight.value = true;
});

watch(quantity, (newQuantity) => {
  if (+newQuantity !== +props.binding.quantity) isSavedQuantity.value = false;
  else isSavedQuantity.value = true;
});

watch(color, (newColor) => {
  if (newColor !== props.binding.color) isSavedColor.value = false;
  else isSavedColor.value = true;
});
</script>
...
<template>
  <div class="flex even:bg-gray-ultra-light border-b h-8 border-black w-fit">
    <p
      class="w-10 border-x border-black text-center"
      :class="
        postone
    ...
    <div
      class="w-24 border-r border-black px-1 flex"
      :class="
        +cut === +props.binding.quantity
          ? 'bg-green-500'
          : +cut === 0 || cut === undefined
