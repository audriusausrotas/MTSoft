...
<template>
  <nav v-if="useUser?.user?.username" class="sticky flex flex-col items-center gap-10 top-8 w-44">
    <div v-if="useUser?.user?.username" class="flex items-center gap-2">
      <NuxtImg src="/icons/menu.svg" width="16" height="16" decoding="auto" />
    ...
    <div>
      ...
      <div v-for="link in menuLinks">
        <NuxtLink :to="link.link" class="flex gap-2 px-4 py-2 rounded-md w-36 hover:bg-red-full hover:text-white"
          ...
          <NuxtImg :src="link.iconPath" width="20" height="20" decoding="auto" />
          {{ link.name }}
        </NuxtLink>
      </div>
    </div>
    ...
    <div>
      ...
      <NuxtLink to="/admin" class="flex gap-2 px-4 py-2 rounded-md hover:bg-red-full hover:text-white hover:bg-dark">
        <NuxtImg src="/icons/users.svg" width="20" height="20" decoding="auto" />
