...
<template>
  <div class="flex gap-1 flex-col">
    <div>
      <p v-if="props.fence.comment" class="w-[312px] text-justify border border-black p-2">
        <span class="font-bold">Komentarai:</span>
        {{ props.fence.comment }}
      </p>
    </div>
    <div
      class="flex flex-col divide-y divide-black border border-black font-semibold xl:text-lg bg-gray-light"
    >
      ...
      <div class="flex text-center divide-x divide-black">
        <div class="flex-1 min-w-fit">
          {{ props.fence.bindings === "Taip" ? "Su apkaustais" : "Be apkaustų" }}
        </div>
    ...
    <div>
      ...
      <div class="flex flex-col flex-1">
        <InstallationFenceInfo
          v-for="(data, index) in props.fence.measures"
          :key="data._id"
          :data="data"
          :index="index"
          :fenceSide="props.fence.side"
          :total="props.fence.measures.length"
