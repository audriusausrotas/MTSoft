...
<template>
  <div
    class="flex flex-col bg-gray-50 justify-between border rounded-lg p-4 max-w-72 w-full gap-2 shadow-lg"
  >
    <div class="flex justify-between items-center gap-2">
      ...
      <div
        ...
        :class="[
          targetPercentage < 100 ? ' bg-green-100 text-green-600' : 'bg-red-100 text-red-600',
        ]"
      >
        <div
          class="w-3 h-3 rounded-full"
          :class="[targetPercentage < 100 ? ' bg-green-500' : ' bg-red-600']"
        ></div>
        {{ targetPercentage < 100 ? "norma" : "viršytas" }}
      </div>
    ...
    <div class="text-xs text-gray-600">{{ name }} - dienos</div>
    ...
    >
      <div v-if="name !== 'Brokas'" class="w-4 h-4">
        <img src="/icons/target.svg" alt="icon" />
      </div>
      <span class="text-gray-400">{{ name === "Brokas" ? "Norma:" : "Tikslas:" }}</span
      >{{ target }} {{ sign }}
    </div>
    <div
      v-if="name === 'Brokas'"
      class="text-lg font-bold"
      :class="targetPercentage < 100 ? 'text-green-600' : 'text-red-600'"
    >
    ...
    <div class="h-2 rounded-full w-full border bg-white overflow-hidden">
      <div
        v-if="name === 'Brokas'"
        :class="[targetPercentage < 100 ? 'bg-green-600' : 'bg-red-600', 'h-full rounded-full']"
    ...
    <div class="flex justify-between text-xs text-gray-400 border-t-2 border-dotted pt-1">
      <div>{{ name === "Brokas" ? "Šiuo metu" : "Liko iki tikslo" }}</div>
      <div v-if="name === 'Brokas'">{{ target - data < 0 ? "Viršytas" : "stabilus" }}</div>
      <div v-else>{{ target - data < 0 ? 0 : target - data }} {{ sign }}</div>
    </div>
  </div>
</template>
