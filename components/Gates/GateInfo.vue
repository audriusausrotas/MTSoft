...
const updateHandler = async (change: string, value: any) => {
  ...
  if (response.success) {
    ...
    if (change === "status") {
      if (
        useUser.user?.accountType !== "Vartonas" &&
        useUser.user?.accountType !== "Gigasta"
      ) {
        const link =
          useUser.user?.accountType === "Vartonas"
        ...
        const data: any = await $fetch("/api/mail", {
          ...
          body: {
            ...
            message: `Užsakymo ${props.gate.orderNr}, ${props.gate.client.address} statusas pakeistas į "${value}". Peržiūrėti galite čia: ${link} "`,
...
<template>
  <div class="flex gap-4 pb-4 flex-wrap items-end">
    <CalcTitle
      ...
      :open="open"
      @click="open = !open"
      class="w-8 h-12 flex justify-center"
    />
    <div v-if="!open" class="flex gap-4 items-end flex-wrap">
      ...
      <BaseInfoField
        ...
        "
        :name="props.gate.manager.split('@')[0].replace('.', ' ')"
        width="w-32"
        label="Atsakingas"
        class="capitalize"
      />
    ...
    <div class="flex flex-col">
      <div
        v-if="!open"
        id="gateStatus"
        class="text-white self-end py-2 rounded-lg w-40 text-center"
        :class="
    ...
    <div class="flex flex-col gap-4 pb-32">
      <div class="flex gap-4">
        ...
        <BaseSelectField
          :values="gateStatus"
          id="gateStatus"
          :defaultValue="props.gate.measure"
          width="w-60"
        ...
        <BaseSelectField
          ...
          id="vartonasUsers"
          :defaultValue="props.gate.manager"
          width="w-60"
      ...
      <div class="flex justify-between font-semibold gap-4 flex-wrap">
        ...
        <div class="flex flex-col gap-4">
          ...
          <BaseInput
            :name="props.gate.client.username"
            width="w-72"
            label="klientas"
            :disable="true"
          />
        ...
        <div class="flex flex-col gap-4">
          ...
          <BaseInput
            :name="props.gate.creator.username"
            width="w-72"
            label="atsakingas asmuo"
            :disable="true"
          />
          <BaseInput
            :name="props.gate.creator.phone"
            width="w-72"
            label="Telefono numeris"
            :disable="true"
          />
          <BaseInput
            :name="props.gate.creator.email"
            width="w-72"
      ...
      <div class="py-8">
        <BaseComment
          :commentsArray="props.gate.comments"
          :id="props.gate._id"
